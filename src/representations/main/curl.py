...
class CURL(CURL_PL):
    ...
    def train_dataloader(self):
        train_dataset = CustomCrafterData(self.trajectories_train, delay=self.delay, **self.conf)
        train_dataloader = DataLoader(train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=10)
        return train_dataloader

    def val_dataloader(self):
        val_dataset = CustomCrafterData(self.trajectories_val, delay=self.delay, **self.conf)
